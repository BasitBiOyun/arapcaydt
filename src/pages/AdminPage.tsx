...
export const AdminPage:React.FC=()=>{
 ...
 const [data,setData]=useState<Overview|null>(null),[error,setError]=useState(''),[message,setMessage]=useState(''),[busy,setBusy]=useState(false),[filter,setFilter]=useState('');
 const load=useCallback(async()=>{setBusy(true);try{const {data,error}=await database().rpc('admin_overview');if(error)throw error;setData(data);setError('');}catch(e){setError(e instanceof Error?e.message:'Yönetim bilgileri alınamadı.');}finally{setBusy(false);}},[]);
 ...
 return <section className="p-6 space-y-6 max-w-7xl mx-auto">
  ...
  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">{[['Öğretmen',data?.members.filter(m=>m.role==='teacher').length||0],['Onay bekleyen',data?.members.filter(m=>m.status==='pending').length||0],['Soru',data?.members.reduce((s,m)=>s+m.questions,0)||0],['Üretilen ses',data?.members.reduce((s,m)=>s+m.voices,0)||0]].map(([label,value])=><div key={label} className="bg-white border rounded-xl p-5"><p className="text-sm text-stone-500">{label}</p><strong className="text-3xl">{value}</strong></div>)}</div>
